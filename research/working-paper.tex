...
\textbf{The headline is a range, not a point.} Our best available measurement of the replication factor $N$ (full-node count) is a \textbf{primary-source lower-bound census ($\geq$32{,}000 known addresses via Bitcoin Core \texttt{getnodeaddresses})} --- the address-manager cap, not a complete enumeration --- while independent estimates span $\sim$10K--100K reachable nodes. Because SCCR is exactly inverse-linear in $N$, the result is stated in three lines (exact figures appear in \S\ref{sec:5}):

\begin{itemize}
\item \textbf{Representative live measurement: $\approx$0.22} --- the average SCCR at $N=32$K, live capture 2026-08-02.
\item \textbf{Observed band: $\approx$0.22--0.29} --- across dated captures at $N=32$K (live, 2026-08-01 dated, and frozen-capture reproduction).
\item \textbf{Model uncertainty: depends strongly on the replication factor $N$} --- the average spans $\sim$0.71 at $N=10$K to $\sim$0.07 at $N=100$K; this $N$-band range ($\sim$0.07--0.71) is a \emph{different} uncertainty from the observed-sample band above and carries the dominant risk.
\end{itemize}
